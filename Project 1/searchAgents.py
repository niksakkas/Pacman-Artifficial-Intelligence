...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Check every point to see if it is visited.. (if it is it will be marked with 1)
        for point in state[1]:
            if point == 0:
                return False
        # ..if all 4 are, return true
        return True
        util.raiseNotDefined()
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if hitsWall == False:
                ...
                # ..check if new point is a corner..
                if (x2, y2) in self.corners:
                    # ..if it is mark it.
                    corners[self.corners.index((x2, y2))] = -1

                cost = 1
                next_point = ((x2, y2), corners)
                next_successor = (next_point, action, cost)
...
def cornersHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        ...
        for x in range(len(state[1])):
            if state[1][x] == 0:
                distance_list.append(manhattanDistance(state[0], corners[x]))
...
def foodHeuristic(state, problem):
    ...
    for food_pos in foodGrid.asList():
        ...
        distances.append(mazeDistance(position, food_pos, start_state))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        # Use uniformCostSearch since the tree is short adn there are many goal states
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        food_points = self.food.asList()

        if state in food_points:
            return True
        else:
            return False
        util.raiseNotDefined()
